...
export function InventoryTurnover({ data }: InventoryTurnoverProps) {
  ...
  const processedData: ITRData[] = Object.values(itrData).map((item) => {
    const avgInventory = (item.totalOpeningStock + item.totalClosingStock) / (2 * item.count)
    const itr = avgInventory > 0 ? item.totalConsumption / avgInventory : 0

    let status: "High" | "Medium" | "Low" = "Medium"
    if (itr > 6) status = "High"
    else if (itr < 2) status = "Low"
  ...
  const avgITR = processedData.reduce((sum, item) => sum + item.itr, 0) / processedData.length
  const highTurnoverCount = processedData.filter((item) => item.status === "High").length
  const lowTurnoverCount = processedData.filter((item) => item.status === "Low").length
      ...
      <CardHeader>
        ...
        <div className="grid grid-cols-3 gap-4 text-sm">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{avgITR.toFixed(2)}</div>
            <div className="text-muted-foreground">Average ITR</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{highTurnoverCount}</div>
            <div className="text-muted-foreground">High Turnover Items</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-red-600">{lowTurnoverCount}</div>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>
                  <Button variant="ghost" onClick={() => handleSort("abcClass")}>
                    ABC Class <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead className="text-right">
                  <Button variant="ghost" onClick={() => handleSort("totalConsumption")}>
                    Total Consumption <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead className="text-right">
                  <Button variant="ghost" onClick={() => handleSort("avgInventory")}>
                    Avg Inventory <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
                <TableHead className="text-right">
                  <Button variant="ghost" onClick={() => handleSort("itr")}>
                    ITR <ArrowUpDown className="ml-2 h-4 w-4" />
                  </Button>
                </TableHead>
            ...
            <TableBody>
              {sortedData.slice(0, 20).map((item) => (
                <TableRow key={`${item.itemId}-${item.itemName}`}>
                  ...
                  <TableCell className="text-right">{item.totalConsumption.toLocaleString()}</TableCell>
                  <TableCell className="text-right">{item.avgInventory.toLocaleString()}</TableCell>
                  <TableCell className="text-right font-mono">{item.itr}</TableCell>
                  <TableCell>
